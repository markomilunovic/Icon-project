...
#previsione finale della % di poter vendere brevemente la casa (6-10)
previsioneVendita = BbnNode(Variable(11, 'previsione vendita', ['si', 'no']), [0.95, 0.05, 0.68, 0.32, 
                                                                                             0.51, 0.49, 0.06, 0.94])
...
bbn = Bbn() \
    ...
    .add_edge(Edge(caratteristiche, previsioneVendita, EdgeType.DIRECTED)) \
    .add_edge(Edge(qualitaCasa, previsioneVendita, EdgeType.DIRECTED))
...
def outputPrediction(tree):
    for node, posteriors in tree.get_posteriors().items():
        if node == 'previsione vendita':
            ...
            if max[1] < 0.3:
                print("Probabilita' di vendita: bassa.\n")
            elif max[1] < 0.45:
                print("Probabilita' di vendita: medio-bassa.\n")
            elif max[1] < 0.6:
                print("Probabilita' di vendita: media.\n")
            elif max[1] < 0.8:
                print("Probabilita' di vendita: medio_alta\n")
            else:
                print("Probabilita' di vendita: alta\n")
